...
Текущий scope оркестратора в MVP (без биллинговой интеграции):

- Фокус на provisioning ассистентов как async-процесс: создание задачи, выполнение worker-ом, выдача финального статуса пользователю.
- Базовые сущности: `assistant_instances`, `provision_jobs`, `provision_steps` (созданный ассистент, job-очередь, пошаговый прогресс).
- Пользовательский контракт: команды `/create_assistant`, `/my_assistants`, `/jobs` для запуска, просмотра списка и проверки статуса задач.
- Статусы job: `queued`, `running`, `needs_input`, `failed`, `completed`; повторный запуск выполняется новой job.
- Интеграция с оплатой/тарифами для orchestrator-пути пока не включена: доступ к orchestration сейчас не зависит от токен-биллинга и не тарифицируется отдельно.

## 11) Текущие ограничения MVP
...
## 12) Напоминания клиентам и алерты команде
